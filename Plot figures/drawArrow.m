%% 绘制带箭头的直线
function drawArrow(V1, V2,arrColor,lineColor,arrowSize,lineWidth)
% 从start_point到end_point画一箭头,arrColor箭头颜色,arrSize，箭头大小
hold on
for ii=1:length(V1)-1
    start_point=[V1(ii),V2(ii)];
    end_point=[V1(ii+1),V2(ii+1)];
%判断参数多少
switch nargin
    case 2
        arrColor  = 'r';
...
arrow           = arrowSize*(start_point' - end_point')/norm(start_point' - end_point');
%使得箭头跟直线长短无关(固定值)
