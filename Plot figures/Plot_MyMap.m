load('SkyHighwaySim_Trajectory_Tf30_M5601.mat')
rh_z=20;
rh1=40;
rh2=15;
...
for tt = 1: 3601
    ...
    for jj=1:5601
    state=zeros(2,1);
    state(1)=floor(UAVstates(tt,16804+jj)/100);
    state(2)=0;
    
    if state(1)~=0 && state(1)~=4
        o2 = UAVstates(tt,3*jj-1:3*jj+1);
        eval(['fs',num2str(jj),'=mydrawcolorball(o2,1,1,1,state);']);
        eval(['temp=[temp fs',num2str(jj),'];'])
        eval(['clear fs',num2str(jj)])
    end
    end
    pause(0.1);
end
